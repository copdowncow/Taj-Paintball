...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] flex">
      <aside className="w-60 hidden md:flex flex-col bg-neutral-900 border-r border-neutral-800">
        <div className="p-5 border-b border-neutral-800">
          <div className="flex items-center gap-1">
            <span className="text-lg font-black text-orange-500">TAJ</span>
            <span className="text-lg font-black text-white">PAINTBALL</span>
          </div>
          ...
          {nav.map(item => (
            <Link key={item.href} href={item.href}
              className={`flex items-center gap-2 px-4 py-3 rounded-xl text-sm font-medium transition-all ${
                pathname === item.href ? 'bg-orange-500 text-white' : 'text-neutral-400 hover:bg-neutral-800 hover:text-white'
              }`}>
              {item.label}
            </Link>
          ))}
        ...
        <div className="p-4 border-t border-neutral-800 space-y-2">
          <Link href="/" className="block text-neutral-500 hover:text-orange-400 text-xs transition-colors">← На сайт</Link>
      ...
      <div className="md:hidden fixed bottom-0 left-0 right-0 z-50 bg-neutral-900 border-t border-neutral-800 flex justify-around py-2">
        {nav.map(item => (
          <Link key={item.href} href={item.href}
            className={`flex flex-col items-center text-xs py-1 px-2 rounded-lg ${pathname === item.href ? 'text-orange-500' : 'text-neutral-500'}`}>
