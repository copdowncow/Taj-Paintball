// Чистый Telegram бот без node-telegram-bot-api
// Только fetch — никакого polling, никаких 409
...
export async function notifyNewBooking(b: Record<string, unknown>) {
  ...
  const date = new Date(b.game_date as string).toLocaleDateString('ru-RU');
  const time = String(b.game_time).substring(0, 5);
  const text =
  ...
  await tgSend(CHAT_ID, text, {
    reply_markup: {
      inline_keyboard: [
        ...
        [
          { text: '🏁 Завершить игру', callback_data: `complete_${b.id}` },
          { text: '🚫 Не пришёл', callback_data: `noshow_${b.id}` },
        ],
...
export async function notifyStatusChange(b: Record<string, unknown>, status: string) {
  ...
  const date = new Date(b.game_date as string).toLocaleDateString('ru-RU');
  await tgSend(CHAT_ID, `🔄 *#${b.booking_number}* — ${labels[status] || status}\n${b.customer_name} · ${date}`);
}

// Обработка нажатий кнопок (вызывается из /api/telegram)
export async function handleCallback(query: {
  ...
  // Разбираем action_bookingId
  const parts = data.split('_');
  const bookingId = parts[parts.length - 1];
  const action = parts.slice(0, -1).join('_');

  type ActionMap = { label: string; status?: string; prepayment?: string; autoConfirm?: boolean };
  ...
  try {
    ...
    if (mapped.prepayment) {
      ...
      // Автоподтверждение брони при получении предоплаты
...
// Регистрация webhook — вызывается один раз при старте
export async function registerWebhook() {
  ...
  if (res.ok) {
    ...
    console.error('Webhook registration failed:', res.description);
  }
}
